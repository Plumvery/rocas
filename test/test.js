...
// マニフェストを焼かなくても ReplicatedStorage のバインディングモジュールから
// 一覧を導けること。
assert(pluginSource.includes("local function discoverAssets()"));
assert(pluginSource.includes("local function parseBindingModule(source)"));
assert(pluginSource.includes("auto-generated by rocas"));

// 一覧に出すのは rocas が同期したアセットだけ。手書きの rbxassetid:// は拾わない
// (拾うと「同期済みアセットの一覧」ではなくなる)。
assert(!pluginSource.includes("discoverFromLooseScript"));
assert(!pluginSource.includes("local function nameFromLine"));
// ロックファイル由来のバインディングを読むので、どこからも使われていない
// アセットも一覧に出る。使用回数で行を絞ってはいけない。
assert(pluginSource.includes("local function matchesFilter(asset)"));
const matchesFilterBody = pluginSource.slice(
	pluginSource.indexOf("local function matchesFilter(asset)"),
	pluginSource.indexOf("local function rebuildUsage"),
);
assert(!matchesFilterBody.includes("usageByAssetKey"));

// rbxm-parser は Source の非 ASCII をラウンドトリップで壊す (NUL が混ざる)。
// テンプレート側のコメントは英語のみに保ち、日本語は JS 側に置く。
const nonAsciiPluginLines = pluginSource.split("\n").filter((line) => /[^\x00-\x7F]/.test(line));
assert.deepStrictEqual(nonAsciiPluginLines, []);
...
// --- Plugin module + loader (source stays in the project) ---
console.log("Testing Studio plugin module and loader generation...");

const pluginModuleSource = generateStudioPluginModule();
// ローダーが require して呼ぶので、関数を返す ModuleScript でなければならない。
assert(pluginModuleSource.includes("return function(context)"));
assert(pluginModuleSource.includes("local widget = context.widget"));
assert(pluginModuleSource.includes("local toggleButton = context.button"));
assert(pluginModuleSource.trimEnd().endsWith("end"));
// 本体は単体版と同じものを埋め込む。
assert(pluginModuleSource.includes("local function discoverAssets()"));
assert(pluginModuleSource.includes("local function cleanup()"));
// モジュール版は自分でツールバーやウィジェットを作らない (リロードで増殖するため)。
assert(!pluginModuleSource.includes("plugin:CreateToolbar"));
assert(!pluginModuleSource.includes("CreateDockWidgetPluginGui"));
assert.deepStrictEqual(pluginModuleSource.split("\n").filter((line) => /[^\x00-\x7F]/.test(line)), []);

const pluginLoaderSource = generateStudioPluginLoader();
assert(pluginLoaderSource.includes("plugin:CreateToolbar(\"rocas\")"));
assert(pluginLoaderSource.includes("CreateDockWidgetPluginGui"));
assert(pluginLoaderSource.includes("RocasPlugin"));
// require はキャッシュするので、リロードのたびにクローンを require する。
assert(pluginLoaderSource.includes("currentModule:Clone()"));
assert(pluginLoaderSource.includes("require(clone)"));
assert(pluginLoaderSource.includes("ServerStorage"));
assert.deepStrictEqual(pluginLoaderSource.split("\n").filter((line) => /[^\x00-\x7F]/.test(line)), []);

// ローダーは .rbxm でも書ける必要がある (Studio が .luau を読まないため)。
const loaderModel = generateStudioPluginRbxmx(pluginLoaderSource);
assert(loaderModel.includes("RocasPlugin"));

assert.strictEqual(
	resolvePluginModuleOutputPath(tempDir),
	path.join(tempDir, "src", "server", "RocasPlugin.luau"),
);
assert.strictEqual(
	resolvePluginModuleOutputPath(tempDir, "custom/Thing.luau"),
	path.resolve(tempDir, "custom/Thing.luau"),
);
assert.strictEqual(
	resolveStudioLoaderOutputPath(tempDir, "custom/loader.rbxm"),
	path.resolve(tempDir, "custom/loader.rbxm"),
);

const writtenModulePath = writeStudioPluginModule(null, tempDir);
assert.strictEqual(writtenModulePath, path.join(tempDir, "src", "server", "RocasPlugin.luau"));
assert.strictEqual(fs.readFileSync(writtenModulePath, "utf8"), pluginModuleSource);

const writtenLoaderPath = writeStudioPluginLoader(null, tempDir, "loader-out/rocas-loader.rbxmx");
