...
const DEFAULT_LOADER_FILE_NAME = "rocas-loader.rbxm";
const DEFAULT_MANIFEST_OUTPUT_PATH = path.join("src", "shared", "RocasManifest.luau");
// ServerStorage 側に置く。プラグインのコードを ReplicatedStorage に置くと
// 本番のクライアントにまで配信されてしまうため。
const DEFAULT_PLUGIN_MODULE_OUTPUT_PATH = path.join("src", "server", "RocasPlugin.luau");
...
function generateStudioPluginModule() {
	return STUDIO_PLUGIN_MODULE_TEMPLATE;
}

function generateStudioPluginLoader() {
	return STUDIO_PLUGIN_LOADER_TEMPLATE;
}
...
function resolvePluginModuleOutputPath(cwd = process.cwd(), outputPath) {
	return path.resolve(cwd, outputPath || DEFAULT_PLUGIN_MODULE_OUTPUT_PATH);
}

function defaultStudioLoaderOutputPath(cwd = process.cwd(), env = process.env, platform = process.platform) {
	const pluginsDir = robloxStudioPluginsDir(env, platform);
	return path.join(pluginsDir || cwd, DEFAULT_LOADER_FILE_NAME);
}

function resolveStudioLoaderOutputPath(cwd = process.cwd(), outputPath) {
	if (outputPath) {
		return path.resolve(cwd, outputPath);
	}

	return defaultStudioLoaderOutputPath(cwd);
}

function writeFileIfChanged(resolvedOutput, source) {
	const outputDir = path.dirname(resolvedOutput);
...
function writeStudioPluginModule(config, cwd = process.cwd(), outputPath) {
	return writeFileIfChanged(resolvePluginModuleOutputPath(cwd, outputPath), generateStudioPluginModule());
}

function writeStudioPluginLoader(config, cwd = process.cwd(), outputPath) {
	const resolvedOutput = resolveStudioLoaderOutputPath(cwd, outputPath);
	const extension = path.extname(resolvedOutput).toLowerCase();
	const loaderSource = generateStudioPluginLoader();
	const source =
		extension === ".lua" || extension === ".luau"
			? loaderSource
			: extension === ".rbxmx"
				? generateStudioPluginRbxmx(loaderSource)
				: generateStudioPluginRbxm(loaderSource);

	return writeFileIfChanged(resolvedOutput, source);
}
...
// ツールバーとウィジェットを作るのは単体プラグイン版だけ。モジュール版は
// ローダーが一度だけ作ったものを context で受け取る。リロードのたびに
// CreateToolbar を呼ぶとツールバーのボタンが増殖するため。
const STUDIO_PLUGIN_HOST = `local toolbar = plugin:CreateToolbar("rocas")
...
const STUDIO_PLUGIN_BODY = `local manifest = { generatedBy = "rocas", mode = "uploaded", assets = {} }
local assets = {}
local localMode = false
local pluginConnections = {}

local function track(connection)
	table.insert(pluginConnections, connection)
	return connection
end
...
local function rebuildUsage()
	...
	-- Invert the tokens once into word -> asset indices, then walk each script
	-- a single time. Testing every script against every asset was
	-- (scripts x assets) plain substring searches: measured at ~3.0s per scan
	-- on a 1600-script place once discovery started finding 424 assets, and it
	-- reran on every source edit. This shape measured ~0.2s on the same place.
	-- Matching whole words also drops the old substring false positives, where
	-- an asset named "star" counted a script that only mentioned "superstar".
	local tokenIndex = {}
	local keysByAsset = {}

	local function indexToken(token, assetIndex)
		if not token then
			return
		end

		-- Tokens like "rbxassetid://123" or "ui/icons/star.png" are split into
		-- their word parts so they can be found by the same single pass.
		for word in string.gmatch(tostring(token), "[%w_]+") do
			if #word >= 4 then
				local bucket = tokenIndex[word]
				if not bucket then
					bucket = {}
					tokenIndex[word] = bucket
				end
				bucket[assetIndex] = true
			end
		end
	end
	...
	for scriptInstance in pairs(watchedScripts) do
		if scriptInstance.Parent ~= nil then
			...
			if source and not string.find(source, "auto-generated by rocas", 1, true) then
				local seen = {}
				for word in string.gmatch(source, "[%w_]+") do
					local bucket = tokenIndex[word]
					if bucket then
						for assetIndex in pairs(bucket) do
							if not seen[assetIndex] then
								seen[assetIndex] = true
								local key = keysByAsset[assetIndex]
...
-- No pre-baked manifest is needed: the asset list comes from the binding
-- modules rocas generates, which are already synced into ReplicatedStorage.
-- Those are built from the lock files, so they list every synced asset --
-- including ones no script references yet -- and the module's variable name
-- gives the asset type while the nested keys give each path.
--
-- Only rocas-generated modules are read. Inline rbxassetid:// literals written
-- by hand elsewhere are deliberately ignored: this browser is for assets rocas
-- synced, and picking up arbitrary ids made the list something else.
--
-- A manifest, when one is present, overrides matching ids afterwards.

local TYPE_NAME_HINTS = {
	{ needle = "image", assetType = "Decal" },
	{ needle = "decal", assetType = "Decal" },
	{ needle = "texture", assetType = "Decal" },
	{ needle = "icon", assetType = "Decal" },
	{ needle = "thumb", assetType = "Decal" },
	{ needle = "sound", assetType = "Audio" },
	{ needle = "audio", assetType = "Audio" },
	{ needle = "music", assetType = "Audio" },
	{ needle = "sfx", assetType = "Audio" },
	{ needle = "anim", assetType = "Animation" },
	{ needle = "video", assetType = "Video" },
	{ needle = "mesh", assetType = "Model" },
	{ needle = "model", assetType = "Model" },
	{ needle = "map", assetType = "Model" },
}

local function assetTypeFromName(value)
	local lowered = string.lower(tostring(value or ""))
	if lowered == "" then
		return nil
	end

	for _, hint in ipairs(TYPE_NAME_HINTS) do
		if string.find(lowered, hint.needle, 1, true) then
			return hint.assetType
		end
	end
...
-- rocas writes one key per line, so reading line by line reconstructs the
-- nested key path exactly.
local function parseBindingModule(source)
	local varName, bodyStart

	for line, position in string.gmatch(source, "([^\\n]*)()") do
		local name = string.match(line, "^local%s+([%w_]+)%s*=%s*{%s*$")
			or string.match(line, "^local%s+([%w_]+)%s*:%s*[%w_]+%s*=%s*{%s*$")
		if name then
			varName = name
			bodyStart = position
			break
	...
	for line in string.gmatch(string.sub(source, bodyStart), "[^\\n]+") do
		local openKey = string.match(line, "^%s*([%w_]+)%s*=%s*{%s*$")
			or string.match(line, "^%s*%[\\"([^\\"]*)\\"%]%s*=%s*{%s*$")

		if openKey then
			depth = depth + 1
			table.insert(stack, openKey)
		else
			local leafKey, leafId = string.match(line, "^%s*([%w_]+)%s*=%s*\\"(rbxassetid://%d+)\\"")
			if not leafKey then
				leafKey, leafId = string.match(line, "^%s*%[\\"([^\\"]*)\\"%]%s*=%s*\\"(rbxassetid://%d+)\\"")
			end

			if leafKey then
				table.insert(stack, leafKey)
...
local function discoverFromBindingModule(moduleScript, source, collect)
	local varName, entries = parseBindingModule(source)
	if not varName or not entries or #entries == 0 then
		return false
	end

	local assetType = assetTypeFromName(varName) or assetTypeFromName(moduleScript.Name) or "Unknown"
	...
	for _, entry in ipairs(entries) do
		collect({
			...
			assetType = assetType,
			source = "binding",
			origin = moduleScript:GetFullName(),
		})
	end
...
local function discoverAssets()
	local discovered = {}
	local byAssetId = {}
	local bindingModules = 0
	local scannedScripts = 0

	local function collect(asset)
		-- The same asset can be bound in more than one group; keep the first.
		if byAssetId[asset.assetId] then
			return
		end

		table.insert(discovered, asset)
		byAssetId[asset.assetId] = asset
	end

	-- Reading Source needs the plugin's script injection permission. Without it
	-- every read fails and the result is indistinguishable from "nothing synced
	-- yet", so count the failures and let the caller say which one happened.
	local unreadable = 0

	for _, descendant in ipairs(ReplicatedStorage:GetDescendants()) do
		if descendant:IsA("ModuleScript") then
			local source = readScriptSource(descendant)
			if not source then
				unreadable = unreadable + 1
			elseif string.find(source, "auto-generated by rocas", 1, true) then
				scannedScripts = scannedScripts + 1

				if not string.find(source, "ROCAS_MANIFEST_JSON_BEGIN", 1, true) then
					if discoverFromBindingModule(descendant, source, collect) then
						bindingModules = bindingModules + 1
					end
				end
			end
		end
	...
	return discovered, byAssetId, bindingModules, scannedScripts, unreadable
end
...
local function applyManifest(nextManifest, sourceModule)
	...
	local discovered, discoveredByAssetId, bindingModules, scannedScripts, unreadable = discoverAssets()

	if localMode then
		-- Local-mode assets have no id yet, so they cannot be merged with scan results.
		assets = manifest.assets or {}
	else
		-- A manifest knows more (group / sourcePath / assetType), so it replaces
		-- matching ids and anything it does not cover stays as scanned.
		for _, asset in ipairs(manifest.assets or {}) do
			local existing = asset.assetId and discoveredByAssetId[asset.assetId]
			if existing then
				for index, entry in ipairs(discovered) do
					if entry == existing then
						discovered[index] = asset
						break
					end
				end
			else
		...
		table.sort(discovered, function(a, b)
			local aGroup = tostring(a.group or "")
			local bGroup = tostring(b.group or "")
...
track(importButton.MouseButton1Click:Connect(importLocalFiles))
track(searchBox:GetPropertyChangedSignal("Text"):Connect(renderList))
...
track(game.DescendantAdded:Connect(watchScript))
track(game.DescendantRemoving:Connect(unwatchScript))
track(ReplicatedStorage.DescendantAdded:Connect(scheduleManifestReload))
track(ReplicatedStorage.DescendantRemoving:Connect(scheduleManifestReload))
...
local function cleanup()
	...
	for scriptInstance, connection in pairs(watchedScripts) do
		pcall(function()
			connection:Disconnect()
		end)
		watchedScripts[scriptInstance] = nil
	end
...
// ローダー。Plugins フォルダに一度だけ置く。中身は「モジュールを探して
// require し直すだけ」なので、以後 rocas が更新しても差し替え不要。
//
// require はモジュールを一度しかキャッシュしないので、リロードのたびに
// クローンを require する。クローンは別インスタンス = 別キャッシュ扱いになる。
const STUDIO_PLUGIN_LOADER_TEMPLATE = `-- This file is auto-generated by rocas. Do not edit manually.
-- Install once into the Roblox Studio local Plugins folder. It only locates
-- and requires the rocas plugin module synced into the place, so it does not
-- need to be regenerated when rocas changes.

local ServerStorage = game:GetService("ServerStorage")
local ReplicatedStorage = game:GetService("ReplicatedStorage")

local MODULE_NAME = "RocasPlugin"
...
placeholder.Name = "RocasLoaderStatus"
placeholder.Size = UDim2.fromScale(1, 1)
placeholder.BackgroundColor3 = Color3.fromRGB(24, 26, 30)
placeholder.BorderSizePixel = 0
placeholder.Font = Enum.Font.Gotham
placeholder.Text = "Looking for " .. MODULE_NAME .. "..."
placeholder.TextColor3 = Color3.fromRGB(190, 196, 206)
...
local function findModule()
	for _, root in ipairs(SEARCH_ROOTS) do
		for _, descendant in ipairs(root:GetDescendants()) do
			if descendant:IsA("ModuleScript") and descendant.Name == MODULE_NAME then
				return descendant
			end
		end
	end
...
reload = function()
	...
	for _, child in ipairs(widget:GetChildren()) do
		if child ~= placeholder then
			child:Destroy()
		end
	end

	currentModule = findModule()

	if not currentModule then
		placeholder.Visible = true
		placeholder.Text = "No " .. MODULE_NAME .. " ModuleScript found in ServerStorage or ReplicatedStorage.\\n\\nRun: rocas plugin --module"
		return
	end

	sourceConnection = currentModule:GetPropertyChangedSignal("Source"):Connect(scheduleReload)

	-- A required module is cached forever, so require a fresh clone instead.
	local clone = currentModule:Clone()
	clone.Parent = currentModule.Parent
	...
	if not ok then
		placeholder.Visible = true
		placeholder.Text = "Failed to load " .. MODULE_NAME .. ":\\n\\n" .. tostring(result)
		return
	end

	if type(result) ~= "function" then
		placeholder.Visible = true
		placeholder.Text = MODULE_NAME .. " must return a function. Regenerate it with: rocas plugin --module"
		return
	end

	local mounted, mountResult = pcall(function()
		return result({
			plugin = plugin,
			toolbar = toolbar,
			button = toggleButton,
	...
	if not mounted then
		placeholder.Visible = true
		placeholder.Text = "Failed to start " .. MODULE_NAME .. ":\\n\\n" .. tostring(mountResult)
		return
	end

	placeholder.Visible = false
	if type(mountResult) == "function" then
		cleanup = mountResult
	end
end
...
for _, root in ipairs(SEARCH_ROOTS) do
	...
	root.DescendantRemoving:Connect(function(descendant)
		if descendant == currentModule then
			scheduleReload()
		end
	end)
end
...
// モジュール版。プロジェクト側に置いて Rojo/Argon でプレースへ同期し、
// ローダープラグインから require させる。ソースはリポジトリ側にあるまま。
const STUDIO_PLUGIN_MODULE_TEMPLATE = [
	"-- This file is auto-generated by rocas. Do not edit manually.",
	"-- Sync this into the place (ServerStorage is recommended) and let the",
	"-- rocas loader plugin require it. Editing it here reloads the window.",
	"",
	STUDIO_PLUGIN_SERVICES,
	"",
	"return function(context)",
	"local plugin = context.plugin",
	"local toolbar = context.toolbar",
	"local toggleButton = context.button",
	"local widget = context.widget",
	"",
	STUDIO_PLUGIN_BODY,
	"end",
	"",
...
module.exports = {
	...
	resolvePluginModuleOutputPath,
	resolveStudioLoaderOutputPath,
