...
- **Folder tree, audio preview, and thumbnails in the asset browser.** The list was flat, so a few hundred assets were hard to navigate. Rows are now grouped into a collapsible folder tree built from the binding modules' nested paths, the way a project view works, with a recursive asset count per folder; searching force-expands so a match is never hidden in a collapsed folder. Audio rows get a play/stop button on the thumbnail: a `Sound` only loads once it has a parent, so it is parented to `CoreGui` (not saved with the place) and played with `Play` rather than `SoundService:PlayLocalSound`, which leaves `IsPlaying` false. Model and Animation rows fetch Roblox's own thumbnail via `rbxthumb://type=Asset`. The colored type square stays behind every thumbnail, so an asset whose thumbnail is missing or still loading still reads as its type.
